...
export default (result) => new Promise((resolve, reject) => {

  const client = new WebTorrent();
  const magnet = result.magnet || result.magnetLink;

  if (!client.get(magnet)) client.add(magnet, torrent => {
    ...
    if (audioExistsIn(torrent)) {
      ...
      resolve(new Album(
        ...
        magnet,
        torrent.ratio,
        files.audio.map(({ name, index }) => Album.newSong(name, index, magnet)),
        files.images,
        torrent.size
      ));
...
const intoImagesAndAudio = (state, file, index) => {
  if (audioFormats.indexOf(path.extname(file.name)) > -1) { state.audio = [...state.audio, { name: file.name, index }] };
  if (imageFormats.indexOf(path.extname(file.name)) > -1) { state.images = [...state.images, file] };
  return state;
}