...
export default (result, array) => new Promise((resolve, reject) => {
...
const intoImagesAndAudio = (state, file, index) => {
  if (audioFormats.indexOf(path.extname(file.name)) > -1) state.audio = [...state.audio, { name: file.name, index, length: file.length }];
